...
void createPositions(glm::vec4 (&sph_vert_pos)[SPH_VERT_COUNT], glm::vec3 pos, float radius){
    //Create rings of vertices for the non-pole vertices
    //These will fill indices 1 - 380. Indices 0 and 381 will be filled by the two pole vertices.
    glm::vec4 v;
    //i is the Z axis rotation
    ...
    for (int i = 0; i < SPH_VERT_COUNT; i++) {
        sph_vert_pos[i] = glm::vec4(sph_vert_pos[i].x * radius + pos.x,
                                    sph_vert_pos[i].y * radius + pos.y,
                                    sph_vert_pos[i].z * radius + pos.z,
                                    1);
    }
}
...
void Sphere::createSphere(glm::vec3 pos, float radius, glm::vec4 col) {
    GLuint sph_idx[SPH_IDX_COUNT];
    glm::vec4 sph_vert_pos[SPH_VERT_COUNT];
    glm::vec4 sph_vert_nor[SPH_VERT_COUNT];
    glm::vec4 sph_vert_col[SPH_VERT_COUNT];

    createPositions(sph_vert_pos, pos, radius);
    createSphereVertexNormals(sph_vert_nor);
    ...
    mp_context->glBufferData(GL_ARRAY_BUFFER, SPH_VERT_COUNT * sizeof(glm::vec4), sph_vert_col, GL_STATIC_DRAW);
