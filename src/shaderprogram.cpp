...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrJointIDs = context->glGetAttribLocation(prog, "vs_Joint_ID");
    attrWeights = context->glGetAttribLocation(prog, "vs_Weight");
    ...
    //skeleton
    for (int i = 0; i < 100; i++) {
        unifBindMatrix[i] = context->glGetUniformLocation(prog, QString("u_BindMatrix[" + QString::number(i) + "]").toStdString().c_str());
        unifJointTransformation[i] = context->glGetUniformLocation(prog, QString("u_JointTransformation[" + QString::number(i) + "]").toStdString().c_str());
    }
}
...
void ShaderProgram::setBindMatrix(std::vector<glm::mat4> bindMatrix) {
    ...
    for (int i = 0; i < bindMatrix.size(); i++) {
        if (unifBindMatrix[i] != -1) {
            context->glUniformMatrix4fv(unifBindMatrix[i], 1, GL_FALSE, &bindMatrix[i][0][0]);
        }
    }
}

void ShaderProgram::setJointTransformation(std::vector<glm::mat4> transMats) {
    useMe();
    for (int i = 0; i < transMats.size(); i++) {
        if (unifJointTransformation[i] != -1) {
            context->glUniformMatrix4fv(unifJointTransformation[i], 1, GL_FALSE, &transMats[i][0][0]);
        }
    }
}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrJointIDs != -1 && d.bindJointID()) {
        context->glEnableVertexAttribArray(attrJointIDs);
        context->glVertexAttribPointer(attrJointIDs, 2, GL_FLOAT, false, 0, NULL);
    }
